...
import Chevron from "@/assets/token/chevron.svg";
  ...
  const content = useMemo(() => {
    return (
      <div className={`flex align-middle gap-1 group ${className}`}>
        <span className="group-hover:text-blue-900">{children}</span>
        <Chevron className="my-auto group-hover:fill-blue-900 fill-black" width="20" height="20"/>
      </div>
    )
  }, [children, className]);
